...
var _ = Describe("Parcel", func() {
	...
	Describe("Generate", func() {
		...
		It("generates resource on root level", func() {
			cmd.Args = append(cmd.Args, "-r", "generate")
		...
		Context("when the commands.sql is ignored", func() {
			BeforeEach(func() {
				args = append(args, "-r", "-i", "commands.sql", "generate")
			})
		...
		Context("when the documentation is disabled", func() {
			BeforeEach(func() {
				args = append(args, "-r", "generate", "-include-docs=false")
			})
			...
			It("does not include API documentation", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				Expect(string(data)).To(ContainSubstring("parcello.AddResource"))
				Expect(string(data)).NotTo(ContainSubstring("Code generated by parcello; DO NOT EDIT."))
			})
		})

		Context("when quite model is enabled", func() {
			BeforeEach(func() {
				args = append(args, "-r", "-q", "generate")
			})
		...
		Context("when the directory is provided", func() {
			BeforeEach(func() {
				args = []string{"-r", "-d", "./database", "generate"}
			})
		...
		Context("when the bundle-dir is provided", func() {
			BeforeEach(func() {
				args = []string{"-r", "-b", "./database", "generate"}
			})
